...
CONST 머리이동속도 = 10
'************************************************
DIM Color_NUM AS BYTE
DIM RAPI_send AS BYTE
DIM TEMP AS BYTE
DIM X9_W9_DATA AS BYTE
DIM X9_DATA AS BYTE
DIM W9_DATA AS BYTE
...
횟수_전진종종걸음_1:
    MOVE G6A,95,  90, 115, 110, 104
    MOVE G6D,104,  81, 146,  89,  102
    MOVE G6B, 85
    MOVE G6C,115
    WAIT
...
횟수_전진종종걸음_2:
    ...
    ENDIF
    적외선측정값 = AD(4)
    IF  적외선측정값 > 벽감지멈춤거리값 THEN GOTO 횟수_전진종종걸음_2_stop

    보행COUNT = 보행COUNT + 1
    IF 보행COUNT > 첫보행횟수 THEN  GOTO 횟수_전진종종걸음_2_stop
...
횟수_전진종종걸음_4:
    MOVE G6D,95,  90, 115, 110, 104
    MOVE G6A,104,  81, 146,  89,  102
    MOVE G6C, 85
    MOVE G6B,115
    WAIT
...
횟수_전진종종걸음_5:
    ...
    적외선측정값 = AD(4)
    IF  적외선측정값 > 벽감지멈춤거리값 THEN GOTO 횟수_전진종종걸음_5_stop

    보행COUNT = 보행COUNT + 1
    IF 보행COUNT > 첫보행횟수 THEN  GOTO 횟수_전진종종걸음_5_stop
...
횟수_전진종종걸음_5_stop:
    ...
    '*********************************************
Color_X9_W9: '
    RAPI_send = 107 + Color_NUM   '0=노랑,   10= 녹색, 20= 빨강
    ETX 4800, RAPI_send
    TEMP = 0
...
Color_X9_W9_WAIT:
    ...
    IF TEMP > 10 THEN
        ...
        X9_DATA = 0
        W9_DATA = 0
        GOTO Color_X9_W9_STOP
    ENDIF

    ERX 4800,X9_W9_DATA, Color_X9_W9_WAIT

    IF X9_W9_DATA > 100 THEN
        W9_DATA = X9_W9_DATA - 100
        X9_DATA = W9_DATA MOD 10
        W9_DATA = W9_DATA / 10
    ELSE
        X9_W9_DATA = 0
        X9_DATA = 0
...
'    '************
    ...
    ERX 4800,A, Line_전진종종걸음_4
    IF A <> A_old THEN
        GOSUB Line_전진종종걸음_2_stop
        RETURN
    ENDIF
...
'
    ...
    ERX 4800,A, Line_전진종종걸음_1
    IF A <> A_old THEN
        GOSUB Line_전진종종걸음_5_stop
        RETURN
    ENDIF
